  ...
  start: async function () {
    ...
    this.timeout = setTimeout(async () => {
      ...
      const sessionImages = await Image.find({
        createdAt: {
          $gte: new Date(Date.now() - config.ama.durationMinutes * 60 * 1000),
        },
      });
      ...
      for (const image of sessionImages) {
        try {
          ...
          console.log(
            `Updated votes for image ${image.messageId}: ${image.upvotes} upvotes, ${image.downvotes} downvotes`
          );
        } catch (error) {
          console.error(
            `Error updating votes for image ${image.messageId}:`,
            error
          );
        }
      }
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  switch (interaction.commandName) {
    ...
    case "mint":
      await mintCommand.execute(interaction);
      break;
  }
});
